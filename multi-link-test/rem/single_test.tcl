...
for {set i 0} {$i < [expr ($router_num - 1)]} {incr i} {
	$ns simplex-link $n($i) $n([expr ($i + 1)]) 10Mbps 60ms REM
	$ns simplex-link $n([expr ($i + 1)]) $n($i) 10Mbps 60ms REM    
}


set remq [[$ns link $n(1) $n(2)] queue]
...
for {set i 0} {$i < [expr ($router_num - 1)]} {incr i} {
	set remq [[$ns link $n($i) $n([expr ($i + 1)])] queue]
